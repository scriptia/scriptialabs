...
import { getAppById, getPerformanceSummary } from '@/server/content-engine';
import { performanceSummaryQuerySchema } from '@/server/validation/content-engine';
...
export async function GET(request: NextRequest) {
  ...
  const parsed = performanceSummaryQuerySchema.safeParse(Object.fromEntries(request.nextUrl.searchParams));
  ...
  const summary = await getPerformanceSummary({ appId: parsed.data.app_id, days: parsed.data.days });
