...
import { getAppById, getStalePublications } from '@/server/content-engine';
import { publicationsQuerySchema } from '@/server/validation/content-engine';
...
export async function GET(request: NextRequest) {
  ...
  const parsed = publicationsQuerySchema.safeParse(Object.fromEntries(request.nextUrl.searchParams));
  ...
  const stale = await getStalePublications({ appId: parsed.data.app_id, staleHours: parsed.data.stale_hours });
