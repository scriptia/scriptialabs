...
| `PIPELINE_RUNNER_TOKEN` | `/api/runs/*`, `/api/bets/*/status` | Lets product-agent's runner claim queued runs and report on them, and lets builder-agent move a finished build to Testing (`POST /api/bets/<slug>/status {"status":"testing"}`). |
