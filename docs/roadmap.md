...
- **Legal & Compliance** — `/privacy`, `/terms`, `/cookies`, `/contact`, `/security`, `/ai-policy` shipped, company-wide (covering all products, present and future) in EN/ES/CA. Merged into the same dynamic route as Product Pages — Next.js doesn't allow two separate dynamic routes at the same URL depth. See [ADR-008](adr/ADR-008-legal-routing.md).

## Next approved phase

**Not yet assigned.** Release tasks were explicitly excluded from the Legal & Compliance phase and remain unscheduled.
...
- **Legal content needs lawyer review before real launch.** The six legal pages are original, well-structured, and modeled on how mature software companies organize this content — but they are not a substitute for review by qualified legal counsel, particularly the governing-law placeholder in Terms of Service and any jurisdiction-specific obligations (GDPR specifics, CCPA, etc.) that depend on where Scriptia Labs is actually registered and who its users are.
- **No consent management platform (CMP) for cookies.** The Cookie Policy documents this as a stated future step, not a current gap being hidden — necessary/functional cookies don't currently need one, but adding analytics or marketing cookies later will.
