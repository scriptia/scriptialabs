...
- **Internal panel V1** — `/internal`, the first backend in the codebase: Neon Postgres + Drizzle, per-user accounts, and bet tracking with typed links (Drive, repo, TikTok and the rest), an append-only update log, manual metric snapshots with trend lines, a kanban board, per-bet tasks, and an audit trail. Replaces the tracking spreadsheet. See [ADR-010](adr/ADR-010-internal-panel.md). (The public site's independence from the database ended with [ADR-013](adr/ADR-013-database-backed-public-content.md), which moved product and legal content into it.)
